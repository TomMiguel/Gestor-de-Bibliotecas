...
export class LoanService {
    ...
    static async getLoanById(id: number): Promise<Loan | undefined> {
        let connection;
        try {
            connection = await getConnection();
            const query = `
    ...
    static async createLoan(loan: Omit<Loan, 'id' | 'user' | 'book'>): Promise<Loan> {
        let connection;
        try {
            connection = await getConnection();
            ...
            const [bookRows]: any = await connection.execute('SELECT id, disponible FROM Libros WHERE id = ?', [loan.id_libro]);
            ...
            const [result]: any = await connection.execute(
                query,
                [loan.id_usuario, loan.id_libro, loan.fecha_prestamo, loan.fecha_devolucion]
            );
    ...
    static async updateLoan(id: number, loan: Partial<Omit<Loan, 'id' | 'user' | 'book'>>): Promise<boolean> {
        let connection;
        try {
            connection = await getConnection();
            await connection.beginTransaction(); // Iniciar una transacción
            ...
            let oldBookId: number | undefined;
            let currentLoanReturnDate: string | null | undefined;

            // Obtener el préstamo actual para verificar si el libro o la fecha de devolución cambian
            const [currentLoanRows]: any = await connection.execute('SELECT id_libro, fecha_devolucion FROM Prestamos WHERE id = ?', [id]);
            ...
            if (loan.id_usuario !== undefined) { fields.push('id_usuario = ?'); values.push(loan.id_usuario); }
            ...
            if (loan.fecha_prestamo !== undefined) { fields.push('fecha_prestamo = ?'); values.push(loan.fecha_prestamo); }
            ...
            if (connection) await connection.rollback(); // Revertir la transacción
    ...
    static async returnBook(loanId: number): Promise<boolean> {
        let connection;
        try {
            connection = await getConnection();
            await connection.beginTransaction();

            const [loanRows]: any = await connection.execute('SELECT id_libro, fecha_devolucion FROM Prestamos WHERE id = ?', [loanId]);
            ...
            const [updateLoanResult]: any = await connection.execute(
    ...
    static async deleteLoan(id: number): Promise<boolean> {
        let connection;
        try {
            connection = await getConnection();
            await connection.beginTransaction(); // Iniciar una transacción
            ...
            const [loanRows]: any = await connection.execute('SELECT id_libro, fecha_devolucion FROM Prestamos WHERE id = ?', [id]);
            if (loanRows.length === 0) {
                return false; // Préstamo no encontrado
            }
            ...
            const [result]: any = await connection.execute('DELETE FROM Prestamos WHERE id = ?', [id]);
